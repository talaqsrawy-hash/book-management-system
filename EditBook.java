...
public class EditBook {
	...
	Button save;
	Button back;
	//create method to return the scene on which that edits a book
	public Scene getScene3(Stage s, Book obj) {
		...
		save.setOnAction(e -> {
			...
			if (o.getBookId() != null && o.getTitle() != null && o.getAuther() != null && o.getCategory() != null
					&& o.getPublishedYear() != 0 && o.getIsBn() != null) {
				for (int i = 0; i < AddNew.list2.size(); i++) {
					if (AddNew.list2.get(i).getBookId().trim().equals(obj.getBookId())) {
						try {
							AddNew.list2.get(i).setAuther(auther.getText());
							AddNew.list2.get(i).setTitle(titel.getText());
							AddNew.list2.get(i).setCategory(cate.getText());
							AddNew.list2.get(i).setPublishedYear(Integer.parseInt(pYear.getText()));
		...
		//to add text field and lable 
		GridPane txt = new GridPane(20, 20);
		txt.add(new Label("book Id:"), 0, 0);
		txt.add(id, 1, 0);
		id.setEditable(false);
		id.setText(obj.getBookId());
		txt.add(new Label("titel:"), 0, 1);
		txt.add(titel, 1, 1);
		titel.setText(obj.getTitle());
		txt.add(new Label("auther:"), 0, 2);
		txt.add(auther, 1, 2);
		auther.setText(obj.getAuther());
		txt.add(new Label("category:"), 0, 3);
		txt.add(cate, 1, 3);
		cate.setText(obj.getCategory());
		txt.add(new Label("publishedYear:"), 0, 4);
		txt.add(pYear, 1, 4);
		pYear.setText(obj.getPublishedYear() + "");
		txt.add(new Label("isBn:"), 0, 5);
		txt.add(iBn, 1, 5);
		iBn.setText(obj.getIsBn());
		iBn.setEditable(false);
